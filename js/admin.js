...
async function loadWorkersAdmin() {
  ...
  document.getElementById('adminWorkersList').innerHTML = (data || []).map(w => {
    ...
    return `<div class="row-item"><div class="row-main"><b>${w.full_name}</b><small>Nivel actual: ${levelInfo(rd?.worker_level).name} · ◈ ${rd?.rate_per_minute}/min · ⭐ ${w.rating}</small></div>
     <div class="row-actions">
       ...
       <button class="btn-small success" onclick="setWorkerRate('${w.id}')">💾 Tarifa</button>
     </div></div>`;
...
async function setWorkerLevel(id, level) {
  ...
  showToast('✅ Nivel ' + l.name + ' asignado (◈ ' + l.rate + '/min)'); loadWorkersAdmin();
}
async function setWorkerRate(id) {
  let rate = parseFloat(document.getElementById('rate_' + id).value);
...
function startKyc(userId, name) {
  ...
  showToast('🎥 Sala abierta. La usuaria entra desde Perfil Pro → Unirse a KYC.');
}
