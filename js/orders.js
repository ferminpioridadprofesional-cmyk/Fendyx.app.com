...
async function loadOrders() {
  ...
  document.getElementById('ordersList').innerHTML = (mine || []).map(o =>
    `<div class="row-item"><div class="row-main"><b>${o.restaurants?.name || 'Restaurante'}</b>
     ...
     ${['ready', 'delivering'].includes(o.status) ? `<small style="color:var(--warning)">🔐 Código: <b>${o.delivery_code}</b></small>` : ''}
     ${o.status === 'delivered' && !o.rated && o.driver_id ? `<button class="btn-small warn" onclick="openRateModal('${o.id}','${o.driver_id}')">⭐ Calificar</button>` : ''}</div>
  ...
  if (isOwner) {
    ...
    if (rest) {
      ...
      document.getElementById('ownerOrdersList').innerHTML = (inc || []).map(o => {
        ...
        return `<div class="row-item"><div class="row-main"><b>${o.profiles?.full_name || 'Cliente'}</b><small>${(o.order_items || []).map(i => i.quantity + 'x ' + i.item_name).join(', ')}</small><small>📍 ${o.delivery_address} · ◈ ${o.total} (tu parte: ◈ ${(o.total * 0.7).toFixed(2)})</small></div><div class="row-actions"><span class="order-status st-${o.status}">${ORDER_LABELS[o.status]}</span>${b}</div></div>`;
...
function openRateModal(orderId, driverId) {
  ...
  if (!m) {
    ...
    m.innerHTML = `<div class="modal-content"><div class="modal-head"><h3>⭐ Calificar entrega</h3><button class="modal-close" onclick="closeModal('modal-rate')">✕</button></div>
      ...
      <select id="rateCategory" class="input-full"><option value="puntualidad">⏱️ Puntualidad</option><option value="trato">😊 Trato</option><option value="cuidado">📦 Cuidado</option><option value="entrega">✅ Entrega correcta</option></select>
      <input type="number" id="rateTip" class="input-full" placeholder="Propina opcional (tokens)" min="0" step="0.5">
      <input type="text" id="rateComment" class="input-full" placeholder="Comentario">
...
async function submitRate() {
  ...
  if (tip > 0) {
    const { data: ok } = await db.rpc('transfer_tokens', { p_from: currentUser.id, p_to: t.driverId, p_amount: tip, p_desc: 'Propina' });
    if (!ok) { showToast('❌ Saldo insuficiente para la propina'); return; }
  }
