...
async function loadAdults(){
  ...
  contentEl.innerHTML='<p class="dim" style="text-align:center;padding:20px">Cargando modelos…</p>';
  let html='';
  for(const cat of CAT_ORDER){
    try{
      const{data,error}=await db.rpc('get_top_models',{p_cat:cat.id,p_days:30,p_limit:4});
      if(error){console.error('Error get_top_models:',error);html+=`<div class="adult-section"><h3>${cat.name}</h3><p class="dim">Error: ${error.message}</p></div>`;continue;}
  ...
  contentEl.innerHTML=html||'<p class="dim" style="text-align:center;padding:20px">Sin modelos disponibles</p>';
}
